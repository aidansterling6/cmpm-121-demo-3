...
const changedCacheKeys: Set<CacheKey> = new Set<CacheKey>();
let locationHistory: LatLng[] = [];
let bUseDeviceLocation: boolean = false;
...
const map: any = leaflet.map(mapElement, {
  center: CurrentPos,
  zoom: STARTZOOM,
  minZoom: MINZOOM,
  maxZoom: MAXZOOM,
...
leaflet.tileLayer(MAPIMAGE, { maxZoom: MAXZOOM, attribution: ATTRIBUTION })
  .addTo(map);
const polyline: any = leaflet.polyline(locationHistory, { color: "red" }).addTo(
  map,
);
let deviceLocation: LatLng = CurrentPos;
const playerMarker: any = leaflet.marker(CurrentPos);
playerMarker.bindTooltip("player position");
playerMarker.addTo(map);

function markCacheChanged(cache: Cache, changedCacheKeys: Set<CacheKey>) {
...
function populateUnknownCell(cell: Cell, key: string) {
...
function getKnownCellCaches(
  cell: Cell,
  key: string,
  knownCachData: CellCachePos,
) {
...
function getCellCaches(cell: Cell): bCellCache {
  const key: string = getCellKey(cell);
  const knownCachData: CellCachePos | undefined = knownCaches.get(key);
  if (!knownCachData) {
    return populateUnknownCell(cell, key);
  } else {
    return getKnownCellCaches(cell, key, knownCachData);
  }
}
function getSavedCell(
  nearbyCaches: string[] | undefined,
  tmpKey: string,
  resultCells: bCell[],
  i: number,
  j: number,
...
function saveOutOfBoundsCells(OrginCell: Cell) {
...
function getCellsNearLatLng(latLng: LatLng): bCell[] {
  ...
  for (
    let i: number = OrginCell.i - CACHESPAWNRANGE;
    i <= OrginCell.i + CACHESPAWNRANGE;
    i++
  ) {
    for (
      let j: number = OrginCell.j - CACHESPAWNRANGE;
      j <= OrginCell.j + CACHESPAWNRANGE;
      j++
    ) {
      const tmpKey: string = getCellKey({ i: i, j: j });
      let bLoadedCaches = false;
      if (savedCaches.has(tmpKey)) {
        const nearbyCaches: string[] | undefined = savedCaches.get(tmpKey);
        if (getSavedCell(nearbyCaches, tmpKey, resultCells, i, j)) {
          bLoadedCaches = true;
        }
      }
      if (!bLoadedCaches) {
  ...
  saveOutOfBoundsCells(OrginCell);
  return resultCells;
}
function collectCoin(cell: Cell, cacheIndex: number): boolean {
  ...
  if (tmpCell) {
    ...
    if (
      ...
      markCacheChanged(cache, changedCacheKeys);
      return true;
    }
  }
  return false;
...
function getDeviceLocation(bUseDeviceLocation: boolean) {
  if (bUseDeviceLocation) {
...
function main() {
  ...
  getDeviceLocation(bUseDeviceLocation);
  loadGameState();
  updatePlayerPosition();
  autoSaveGame();
}
